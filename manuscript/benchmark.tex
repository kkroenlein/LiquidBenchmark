...
This inaccessability hinders reproducible benchmarks of forcefield accuracy.
...
Although small molecule forcefields have also been the subject of benchmarks~\cite{caleman2011force} and improvements~\cite{fennell2014fixed}, such work has focused on small perturbations to specific functional groups.  
...
Because this deviation likely stems from the lack of electronic polarization, we added a simple empirical correction for polarization \cite{bosque2002polarizabilities} that is based on counting the number of atom types in a molecule:
$$\alpha = 1.53 n_C + 0.17 n_H + 0.57 n_O + 1.05 n_N + 2.99 n_S + 2.48 n_P + 0.22 n_F + 2.16 n_{Cl} + 3.29 n_{Br} + 5.45 n_I + 0.32$$
From the polarizability, one can correct the static dielectric using the following equation (from ref. \cite{horn2004}):
$$\epsilon_{corr} = \epsilon_{MD} + 4 \pi N  \frac{\alpha}{\langle V \rangle}$$
...
(b).  A congeneric series of chloromethanes show dielectric constants between 2 and 10.  
}
...
However, the model for $\mathrm{CHCl_3}$ uses fixed point charges to account for \emph{both} the net (permanent) dipole moment and the (electronic) polarizability, whereas the $\mathrm{CCl_4}$ model contains no treatment of polarizability.  
...
Second, ThermoML is actively growing, through partnerships with several journals (Tab. \ref{table:journals})--new experimental measurements published in these journals are critically examined by the TRC and included in the archive.  
...
\bottomrule
\end{tabular}
\caption{List of journals currently involved in ThermoML data archival at NIST/TRC.
}
