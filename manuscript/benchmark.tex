...
{\color{blue}[KAB: So the Github page for the paper will have CSV files for the processed dataset.  I wonder if that will be sufficient.  Alternatively, we can put those CSV files in the SI.  The advantage of github is that the data lives with the code, which could help reproducibility.]}
...
AM1-BCC~\cite{am1bcc1,am1bcc2, velez2014time} charges were generated with the OpenEye Python Toolkit version 2015-2-3 \cite{openeye}, using the {\tt oequacpac.OEAssignPartialCharges} module with the {\tt OECharges\_AM1BCCSym} option, which utilizes a conformational expansion procedure prior to charge fitting to minimize artifacts from intramolecular contacts.  
...
{\color{blue}[KAB: This would depend on the individual molecule, temperature, and (?) platform, so for now I've mentioned that the parameters are automatically chosen.]}
...
With this protocol, we found starting trajectory lengths of $14000 \pm 11000$ frames (250 fs each), discarded regions of $1800 \pm 5600$, and statistical inefficiencies of $30 \pm 34$ (mean $\pm$ standard deviation).  
...
{\color{blue}[KAB: I think our data is too big, we're on the order of 50GB.]}
...
For a given compound, the errors are roughly similar at different temperatures (Fig. ~\ref{figure:AllDensities}).  Overall, the predictions give an average density of $947 \pm 9$ $kg / m^3$, while the measurements give an average density of $928 \pm 8$ $kg / m^3$.  

The largest density errors occur for 1,4-dioxane, 2,5,8-trioxanonane, 2-aminoethanol, dimethyl carbonate, formamide, and water.  The absolute error on these poor predictions is on the order of 50 $kg / m^3$, which is substantially higher than the measurement error ($\le 8$ $kg / m^3$; see Fig.~\ref{figure:ErrorAnalysisDensity}).  
...
{\color{blue}[KAB: We should discuss this more---it's not clear what you're asking.]
}
}
...
\shellcmd{conda install "gaff2xml==0.6.4" "pymbar==2.1" "mdtraj==1.3" "openmm==6.3" packmol}
%\end{lstlisting}
