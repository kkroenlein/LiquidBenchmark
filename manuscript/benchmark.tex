...
% JDC: What do we think about the title?  Do we want to tweak this a bit to stress the ThermoML aspect?
% e.g "Toward automated benchmarking of atomistic molecular forcefields: Using the ThermoML Data Archive as a machine-readable source of physical properties"?
...
{\color{red}[JDC: Aren't there several ThermoML extension papers to cite as well?]}
...
In order to ensure stable automated equilibration, PackMol box volumes were chosen to accommodate twice the van der Waals volume of the enclosed atoms.  
{\color{red}[JDC: Did you use standard van der Waals radii, or did you use the $\sigma$ parameter of Lennard-Jones?]}
...
Simulation boxes were first minimized {\color{red}[JDC: Describe minimization procedure?]} and equilibrated for $10^7$ steps with an equilibration timestep of 0.4 fs and a collision rate of 5 ps$^{-1}$.  
...
Equilibration and production simulations utilized a Monte Carlo barostat with a control pressure of 1 atm, utilizing molecular scaling and automated step size adjustment during equilibration, with volume moves attempted every 25 steps.  
...
Production simulations were continued until automatic analysis showed standard errors in densities were less than $2 \times 10^{-4}$ g / mL.
...
{\color{red}[JDC: Standard deviations are not very useful here since they are often larger than the mean and these distributions are highly non-normal.  Suggest you quote 95\% confidence intervals or quartiles instead of standard deviations.]}
...
\end{eqnarray}
where $\beta \equiv k_B T$ is the inverse temperature.
...
{\color{red}[JDC: Check that this is accurate}
...
{\color{red}[JDC: Bas has a nice way of auto-formatting numbers, like 136 212.]}
}
...
For a given compound, the errors are roughly similar at different temperatures (Fig.~\ref{figure:AllDensities}).  
...
{\bf Outliers.}
The largest density errors occur for a number of oxygen-containing compounds: 1,4-dioxane; 2,5,8-trioxanonane; 2-aminoethanol; dimethyl carbonate; formamide; and water.  
{\color{red}[JDC: Could we make a figure of just these compounds, along with their average errors?]}
The absolute error on these poor predictions is on the order of 0.05  g/mL, which is substantially higher than the measurement error ($\le0.008$ g/mL; see Fig.~\ref{figure:ErrorAnalysisDensity}).  
...
We therefore compare simulations against tmeasurements in our ThermoML extract on the $1/\epsilon$ scale in Fig.~\ref{figure:Dielectric}.  
