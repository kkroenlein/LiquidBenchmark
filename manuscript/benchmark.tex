...
Simulation boxes containing 1000 molecules were constructed using PackMol version 14-225~\cite{martinez2009packmol} wrapped in an automated tool.
{\color{red}[JDC: What density was used during the packmol step?]}
{\color{red}[JDC: Remind readers how they can obtain the tool?]}
AM1-BCC~\cite{am1bcc1,am1bcc2} charges were generated with the OpenEye Python Toolkit version 2014-6-6 \cite{openeye}, using the {\tt oequacpac.OEAssignPartialCharges} module with the {\tt OECharges\_AM1BCCSym} option, which utilizes a conformational expansion procedure prior to charge fitting to minimize artifacts from intramolecular contacts.  
...
\shellcmd{conda install "gaff2xml>=0.7" "pymbar>=2.1" "mdtraj>=1.3" "openmm>=6.3" packmol}
%\end{lstlisting}
{\color{red}[JDC: Maybe we can move these instructions on installing the exact versions to the Appendix or SI, where we can go into one-column mode?  Also, we probably want to specify how to get the \emph{exact} versions using == rather than the \emph{latest} versions using >=.]}
...
Automatic analysis was run every {\color{red}X ps of simulation time}, and utilized the {\tt detectEquilibration} method in the timeseries module of pymbar 2.1~\cite{shirts2008statistically} to automatically trim the initial portion of the simulation with strong far-from-equilibrium behavior by maximizing the number of effectively uncorrelated samples in the remainder of the production simulation as determined by autocorrelation analysis.
...
{\color{red}[JDC: I've tried to clarify this. Can you check if I did this correctly?]}
...
Static dielectric constants were calculated using the dipole fluctuation approach appropriate for PME with conducting (``tin-foil'') boundary conditions~\cite{horn2004}, with the total system box dipole $\mu$ computed from trajectory snapshots using MDTraj 1.3~\cite{mcgibbon2014mdtraj}.
{\color{red}[JDC: I think the TIP4P-Ew paper cites the source of this equation, which we should cite as well.]}
\begin{eqnarray}
...
{\color{red}All custom code is available from XXX.}
...
\begin{enumerate}
 ...
 \item Measured mass densities  $\le$ 300 kg m$^{-3}$ were discarded to eliminate gas-phase measurements
...
% KAB: This is the actual order the filters are applied.  
}
...
%KAB: Previous study does not appear to have an uncertainty, at least according to Table 2 in said work.

{\color{red}[JDC: Discuss outliers from Fig.~\ref{figure:AllDensities} here.  There must be more things we can say about densities.  
Some of the densities are quite good, while others (e.g.~water, sulfolane, dimethylcarbonate) seem poor, with systematic bias toward higher densities than experiment.  
We know, for example, for sulfolane, hypervalent sulfur atoms are a challenge for GAFF (from SAMPL challenges), and dimethyl carbonate has \emph{three} oxygens, which had a vdW issue also observed in a previous SAMPL (and maybe addressed in the dielectric paper?).
We can also point out that densities at different temperatures for a given molecule seem to be biased in a consistent way.  
Perhaps we could start by producing a figure showing OpenEye OEDepict images of the outliers, or embed those images in Fig.~\ref{figure:AllDensities}?]}
...
\color{red}
JDC: Can we say anything about mass densities?
For example, how accurate do we expect mass densities need to be?
Are there other properties we expect to be highly correlated to mass densities?
...
Fixed-charge forcefields give $\epsilon \approx 1$ for each species; for example, we calculated $\epsilon = 1.003 \pm 0.0002$ for octane. {\color{red}[JDC: Sig figs issue.]}  
...
%KAB: I can't make this figure with vector graphics, as it involved a screenshot from an online chemical editor.  
%{\color{red}[JDC: Can we show both experimental and GAFF/AM1-BCC computed dielectric constants for some of these compounds?]}
%KAB: now reporting the calculated value for octane, the only one in our list.
}
...
{\color{red}[JDC: Unfinished thought?]}
